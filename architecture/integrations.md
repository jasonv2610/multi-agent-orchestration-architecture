...
User-specific data is never shared across cache contexts. The user context hash is an opaque scope identifier and is not exposed to agent logic.

**Registry resolution:** No workflow contains a raw data store ID. All references use logical names (e.g., `operations_sheet`, `knowledge_base`) resolved at runtime through the registry. Changing a data store requires one registry update. No workflow modifications are needed.
...
**Design note:** The communication layer is treated as a pure I/O concern. All content decisions,what to say, how to format, what to retrieve,happen inside the orchestration and agent layers. The communication interface receives a formatted response and delivers it. It has no influence on routing or agent logic.
...
| Tool | Role in System |
|------|---------------|
| Claude Code via MCP | Developer interface to the orchestration system. Supports workflow inspection, registry queries, and configuration changes through natural language during development and debugging sessions. Does not participate in production routing. |
| Git | Version control for all workflow files, contracts, registry, and governance documents. Pre-commit validation gates check contract compliance and version increments before changes reach the repository. |
...
| Cache (Redis) | Cache miss: agent runs normally; cache failure does not block agent execution |
