...
### Added
- `package.json` — Dev dependency manifest: `ajv`, `ajv-formats`, `semver`, `jest`, `husky`; npm scripts for `test`, `test:contract`, `test:schema`, `test:policy`, `validate`, `prepare`
...
- `examples/README.md` — Index of all example resources with pattern coverage matrix
...
- `governance/pre-commit-contract-validator.js` — Structural example of the pre-commit validation script; illustrates version bump checking and contract compliance validation at commit time
- `tests/README.md` — Testing approach: contract compliance, payload schema, and policy compliance test categories
...
### Changed
- `examples/scheduling-assistant/manifest.json` — Version bumped from `v1` to `v1.1.0` to match workflow file versions after Merge node fix
- `examples/scheduling-assistant/workflows/06_notify.json` — Email Send node disabled by default and converted to a configurable placeholder; note added directing implementers to replace with their delivery channel (Telegram, webhook, Slack, etc.)
- `governance/contracts-overview.md` — Added contract example cross-references to the enforcement section
- `.gitignore` — Expanded from single entry to comprehensive pattern set (env files, OS artifacts, editor directories, build outputs)

### Fixed
- `examples/scheduling-assistant/README.md` — Removed internal project reference from footer
...
- `registry-pattern/ssot-pattern.md` — Added missing blank lines around paragraph between code block and section heading
